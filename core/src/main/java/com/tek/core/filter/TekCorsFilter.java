 ...
 * Cors Filter Policy
 * <p>
 * 1) Access-Control-Allow-Origin can't accept a wildcard <*>
 * if the requesting client sends <i>withCredentials</i> header: true
 * <p>
 * 2) Access-Control-Allow-Origin can't send multiple origins (except using the wildcard <*>)
 * <p>
 * 3) Access-Control-Allow-Headers OPTIONS must return [HttpServletResponse.SC_OK]
 * to avoid 401 status code with authorization header
 * <p>
 * 4) Access-Control-Allow-Credentials must be true
 * if the requesting client sends <i>withCredentials</i> header: true
 *
 * @author MarcoPagan
 */
